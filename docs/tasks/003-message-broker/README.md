...
## SRS - Requirements
- [x] Kafka 2.8+ hoặc NATS 2.0+ trong Docker Compose.
- [x] Topics/Subjects: market.*, strategy.*, risk.*, order.*, execution.*, portfolio.*, notification.*.
- [x] Version hóa schema event (v1, v2).
- [x] Producer: KafkaProducer hoặc NatsConn abstraction.
- [x] Consumer: consumer group, offset tracking, retry/DLQ.
- [x] Partitioning: theo user_id hoặc symbol để scale.
- [x] At-least-once delivery guarantee.

## PRD - Acceptance Criteria
- [x] Chạy `docker-compose up` broker lên without error.
- [x] Produce message tới topic -> consume được message.
- [x] Consumer group tracking offset -> restart consumer không duplicate.
- [x] Dead-letter queue (DLQ) catch invalid message.
- [x] Multi-partition topic: producer distribute balanced.
